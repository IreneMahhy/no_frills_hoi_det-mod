...
class Features(Dataset):
    ...
    def get_faster_rcnn_prob_vecs(self, hoi_ids, human_probs, object_probs):
        ...
        if isinstance(self.const, FeatureConstantsVcoco):  # 过滤不存在role的动作的object score
            ...
            for i, hoi_id in enumerate(hoi_ids):
                ...
                for hoi_id_, hoi in self.hoi_dict.items():
                    if hoi['object']:  # 如果该动作存在role
                        object_prob_vecs[i, int(hoi_id_)-1] = object_probs[i]
        else:
    ...
    def get_obj_one_hot(self, hoi_ids):
        num_cand = len(hoi_ids)
        obj_one_hot = np.ones([num_cand, len(self.obj_to_id)])
    ...
    def __getitem__(self, i):
        ...
        to_return = {
            ...
            'hoi_label_vec': hoi_label_vecs,  # 将每个候选对的hoi_label标示扩展到N个hoi类别上
            'box_feat': box_feats,
        ...
        if isinstance(self.const, FeatureConstantsVcoco):
            to_return['object_one_hot'] = np.ones([len(to_return['hoi_id']), len(self.obj_to_id)])
        else:
